...
### Added
- 세션 삭제 전에 앱 내부 확인창을 표시하고, 확인 후 삭제가 끝날 때까지 로딩 상태를 보여주도록 추가.
- 세션 목록 헤더 체크박스로 현재 표시된 세션을 전체 선택/해제할 수 있도록 추가.
- 오른쪽 상세 패널 하단에 Codex CLI 상태, 모델, 추론 설정, 공식 usage 페이지 이동 버튼을 표시하는 Codex 상태 패널을 추가.
- 이전 작업 맥락을 보존하기 위해 handover 문서 2개를 저장소에 추가.

### Changed
- 체크박스와 주요 버튼 색상을 차가운 시안 블루 계열 테마로 정리.
- README 스크린샷을 실제 앱 화면 기준으로 갱신하고, 개인 작업 내용은 모자이크 처리.

### Removed
- 안정적으로 동작하지 않는 실험적 `/status` 자동 조회 버튼과 관련 백엔드 명령을 제거.

## [0.5.6] — 2026-06-25

### Changed
- 세션 목록 컬럼 순서를 이름, 마지막 활동, 설명, 크기, 프로젝트, ID 순으로 재배치해 최근 작업 흐름을 더 빨리 훑어볼 수 있도록 변경.
- 창이 좁아질 때 컬럼을 압축하지 않고 테이블 가로 스크롤로 탐색하도록 조정.
- README 예시 스크린샷을 최신 UI의 최대화 화면 기준으로 갱신.
